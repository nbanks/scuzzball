...
{$M 16384,0,655360}
 ...
 var BoardSeg,PiecesSeg,ScreenBuf:Word;
     ...
     TextScreen:Array[0..24,0..79] of CharColour Absolute $B800:$0000;
     MonoScreen:Array[0..24,0..79] of CharColour Absolute $B000:$0000;
 Procedure GetMems;
 Begin
   Asm
 ...
 End;
 Procedure DrawTextPiece(X,Y:Byte; Colour:Byte; ReplaceBackground:Boolean);
  Const Piece:Array[0..2,0..5] of Char=
              ('  ÜÜ  ',
               ' ÞÛÛÝ ',
               '  ßß  ');
        MonoKing:Array[0..2,0..5] of Char=
                 ('  ÜÜ  ',
                  ' ÞÝÞÝ ',
                  '  ßß  ');
  var SubX,SubY:Byte;
      MonoColour:Byte;
      MonoUse:Array[0..2,0..5] of Char;
 Begin
   If GfxBackground=0 then
     For SubY:=0 to 2 do
       For SubX:=0 to 5 do
         With TextScreen[Y+SubY,X+SubX] do
         Begin
           If ReplaceBackground then Co:=Colour
           Else
             If Colour<>0 then
               Co:=(Co And $F0) or (Colour And $0F)
             Else
               Co:=(Co And $F0) or (Co SHR 4);
           Ch:=Piece[SubY,SubX];
         End
   Else
   Begin
     If Colour SHR 4=$F then
     Begin
       If Colour And $F<$8 then
       Begin
         MonoScreen[Y  ,X].Ch:='Ú';
         MonoScreen[Y+1,X].Ch:='³';
         MonoScreen[Y+2,X].Ch:='À';
         MonoScreen[Y  ,X+5].Ch:='¿';
         MonoScreen[Y+1,X+5].Ch:='³';
         MonoScreen[Y+2,X+5].Ch:='Ù';
       End Else
       Begin
         MonoScreen[Y  ,X].Ch:='É';
         MonoScreen[Y+1,X].Ch:='º';
         MonoScreen[Y+2,X].Ch:='È';
         MonoScreen[Y  ,X+5].Ch:='»';
         MonoScreen[Y+1,X+5].Ch:='º';
         MonoScreen[Y+2,X+5].Ch:='¼';
       End;
     End Else
     Begin
       If Colour=$44 then Colour:=1;
       Case Colour and $F of
         0:Begin
             FillChar(MonoUse,SizeOf(MonoUse),0);
             MonoColour:=7;
           End;
         4:Begin
             Move(Piece,MonoUse,SizeOf(MonoUse));
             MonoColour:=7;
           End;
         7:Begin
             Move(Piece,MonoUse,SizeOf(MonoUse));
           ...
           End;
         $C:Begin
              Move(MonoKing,MonoUse,SizeOf(MonoUse));
              MonoColour:=$7;
            End;
         $F:Begin
              Move(MonoKing,MonoUse,SizeOf(MonoUse));
            ...
            End;
       Else
         FillChar(MonoUse,SizeOf(MonoUse),' ');
         MonoColour:=$70;
       End;
       For SubY:=0 to 2 do
         For SubX:=0 to 5 do
           With MonoScreen[Y+SubY,X+SubX] do
           Begin
             If ReplaceBackground then
               Co:=MonoColour
             Else
               If ((Y+SubY) div 3) and 1=((X+SubX-16) div 6) and 1 then
                 Co:=$70 {Checks to see if it's a blank square.}
               Else Co:=MonoColour;
             Ch:=MonoUse[SubY,SubX];
           End;
     End;
   End;
 End;
       ...
       Begin
         ...
         Else
           ...
           Else
             Colour:=$00;
           End;
         DrawTextPiece(X*6+16,Y*3,Colour,True);
       End
   Else {Graphix}
   Begin
 ...
 Procedure DrawPieces(XSkip1,YSkip1,XSkip2,YSkip2:Byte);
  ...
  var X,XStart,Y,YStart:Byte;
      ...
      BlankPiece:Byte;
 Begin
   If GfxBackground<2 then Exit; {TextMode}
 ...
 Procedure MovePiece(X1,Y1,X2,Y2:Byte; Jump:Boolean);
  var X,Y,Width,Height,Tall,
      ...
      Count,MidX,MidY,Player,SHRVal1,SHRVal2,SHRVal3,
      TextX,TextY,TextColour:Byte;
 Begin
   If GfxBackground<2 then {TextMode}
   Begin
     If X1=8 then Exit;
     TextX:=X1*6+16;
     TextY:=Y1*3;
     ...
     Begin
       TextColour:=0;
       DrawTextPiece(TextX,TextY,0,False);
       Case Board[Y1 SHL 3 or X1] of
         Red:TextColour:=4;
         RedKing:TextColour:=$C;
         Black:TextColour:=7;
         BlackKing:TextColour:=$F;
       End;
       If Count=1 then
       Begin
         If (TextColour=4) and (Y2=0) then TextColour:=$C; {King Me!}
         If (TextColour=7) and (Y2=7) then TextColour:=$F; {King Me!}
       End;
       If X1>X2 then Dec(TextX)
       Else Inc(TextX);
       If (Count and 1=0) then
         If (Y1>Y2) then Dec(TextY)
         Else Inc(TextY);
       DrawTextPiece(TextX,TextY,TextColour,False);
       Delay(TextSpeed);
       Dec(Count);
     End;
     Exit;
 ...
 Procedure PutCursor(X,Y,Colour:Byte);
  var XPos,YPos,WantedColour:Byte;
 Begin
   If GfxBackground<2 then
   Begin
     If (Y Xor X) And 1=0 then {The colour is red.}
       WantedColour:=$44 {Red on Red}
     Else
       Case Board[Y SHL 3+X] of
         Black:WantedColour:=$07; {Grey on Black}
         Red:WantedColour:=$04; {Red on Black}
         BlackKing:WantedColour:=$0F; {White on Black}
         RedKing:WantedColour:=$0C; {Bright red on Black}
       Else
         WantedColour:=$00;
       End;
     If Colour<>0 then
       If WantedColour in[0,$44] then
         WantedColour:=Colour or (Colour SHL 4)
       Else
         WantedColour:=WantedColour or (Colour SHL 4);
     If (GfxBackground=0) or (Colour=0) then
       DrawTextPiece(X*6+16,Y*3,WantedColour,True)
     Else
       If Colour and CursorColour=CursorColour then
         DrawTextPiece(X*6+16,Y*3,$FF,True)
       Else
         DrawTextPiece(X*6+16,Y*3,$F0,True);
   End Else
   Begin
   ...
   End;
 End;
 Type MoveType=Array[0..4] of Byte;
 ...
 Procedure PlayBestMove;
  ...
  Begin
    Case Board[NewSpot] of
      Black:Improvement:=(NewSpot-OldSpot) and $38-CurDepth+40;
      Red:Improvement:=(NewSpot-OldSpot) and $38+CurDepth-40;
      BlackKing:
      Begin
  ...
  Begin
    ...
    Begin
      TryMove:=-$2800+Word(CurDepth) SHL 8; {The sooner/later the better.}
      Exit;
    End;
    If LastRed=$FF then {Black wins}
    Begin
      TryMove:=$2800-Word(CurDepth) SHL 8;
      Exit;
    End;
    If (CurDepth in[1,3]) and Stalemate then
    Begin {Really bad for the computer.}
      If Red then
        If LastRed<LastBlack then TryMove:=-$1000
        Else TryMove:=-$100
      Else
        If LastRed>LastBlack then TryMove:=$1000
        Else TryMove:=$100;
    ...
    If Red then
    Begin
      ThisBestScore:=$5000-Word(CurDepth) SHL 8; {Worst (No Moves)}
          ...
          Begin
            ...
            Begin
              ...
              Begin
                ...
                CurImprovement:=-$100+CurDepth SHL 3+
    ...
    End Else {Black}
    Begin
      ThisBestScore:=-$5000+Word(CurDepth) SHL 8; {Worst (It loses)}
    ...
    Begin
      ...
      Begin
        ...
        Begin
          ...
          If TakenPieceType>1 then {It was a king.}
            Dec(CurScore,$300-CurDepth SHL 1-Ord(LastRed<LastBlack) SHL 3)
          Else {It was normal}
            Dec(CurScore,$300-CurDepth SHL 1-Ord(LastRed<LastBlack) SHL 3);
          CurImprovement:=
          ...
          End Else{Continues jumping...}
            TryJumps(True,Moves[Pos],CurDepth,BestScore,CurScore,Alpha,Beta);
          If TakenPieceType>1 then {It was a king.}
            Inc(CurScore,$300-CurDepth SHL 1-Ord(LastRed<LastBlack) SHL 3)
          Else {It was normal}
            Inc(CurScore,$200-CurDepth SHL 1-Ord(LastRed<LastBlack) SHL 3);
      ...
      Begin {It's not jumping any more.}
        ...
        Begin {Kings are good.}
          Inc(CurScore,-$100+CurDepth SHL 3);
    ...
    Begin {Black}
      ...
      Begin
        ...
        Begin
          ...
          Dec(LastRed);
          If TakenPieceType>1 then {It was a king.}
            Inc(CurScore,$300-CurDepth SHL 1-Ord(LastBlack<LastRed) SHL 3)
          Else {It was normal}
            Inc(CurScore,$200-CurDepth SHL 1-Ord(LastBlack<LastRed) SHL 3);
          CurImprovement:=
          ...
          End Else{Continues jumping...}
            TryJumps(False,Moves[Pos],CurDepth,BestScore,CurScore,Alpha,Beta);
          If TakenPieceType>1 then {It was a king.}
            Dec(CurScore,$300-CurDepth SHL 1-Ord(LastBlack<LastRed) SHL 3)
          Else {It was normal}
            Dec(CurScore,$200-CurDepth SHL 1-Ord(LastBlack<LastRed) SHL 3);
          Dec(CurScore,CurImprovement);
 ...
 Begin
   ...
   Repeat
     ...
     Dec(CurPower);
   Until (BestMoveLength<>0) or (CurPower=0);
     {If it dosen't have a move, try again.}
   If BestMoveLength=0 then Exit;
